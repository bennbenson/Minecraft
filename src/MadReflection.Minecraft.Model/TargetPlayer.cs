...
{
	public abstract class TargetPlayer : Target, IEquatable<TargetPlayer>
	{
		...
		public static TargetPlayer Parse(string s) => InternalTryParse(s, out TargetPlayer? result, out Exception? exception, true) ? result : throw exception!;

		public static bool TryParse(string s, [NotNullWhen(true)] out TargetPlayer? result) => InternalTryParse(s, out result, out _, false);

		public static bool InternalTryParse(string s, [NotNullWhen(true)] out TargetPlayer? result, out Exception? exception, bool needException)
		{
			if (s is null)
			{
				result = null;
