...
namespace Minecraft.Model.Bedrock
{
	...
	public readonly struct RelativeFloat : IEquatable<RelativeFloat>, IArgumentText
	{
		...
		private static bool InternalTryParse(string s, out RelativeFloat result, out Exception? exception, bool needException)
		{
			if (s is null)
			{
				result = default;
		...
		public static explicit operator float(RelativeFloat value) => value.Type == PositionType.Absolute ? value.Value : throw new InvalidCastException($"Cannot cast a relative or local {nameof(RelativeFloat)} instance to {nameof(Single)}.");
