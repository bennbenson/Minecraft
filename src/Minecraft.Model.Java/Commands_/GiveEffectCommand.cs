...
namespace Minecraft.Model.Java
{
	...
	{
		public GiveEffectCommand(TargetEntity target, StatusEffect effect)
			: this(target, effect, null, 0, false)
		{
		}

		public GiveEffectCommand(TargetEntity target, StatusEffect effect, int seconds)
			: this(target, effect, seconds, 0, false)
		{
		}

		public GiveEffectCommand(TargetEntity target, StatusEffect effect, int seconds, int amplifier)
			: this(target, effect, (int?)seconds, amplifier, false)
		{
		}

		public GiveEffectCommand(TargetEntity target, StatusEffect effect, int seconds, int amplifier, bool hideParticles)
			: this(target, effect, (int?)seconds, amplifier, hideParticles)
		{
		}

		private GiveEffectCommand(TargetEntity target, StatusEffect effect, int? seconds, int amplifier, bool hideParticles)
			: base("effect")
		{
			if (target is null)
		...
		{
			StringBuilder result = new StringBuilder($"/effect give {Target.GetArgumentText()} {Effect.GetArgumentText()}");
