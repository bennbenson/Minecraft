...
namespace Minecraft.Construction.Bedrock
{
	public static class NetherPortalRoom
	{
		...
		public static IEnumerable<Command> GenerateNetherPortalRoomX(NetherPortalRoomParameters parameters, Block block)
		{
			...
			yield return new FillCommand(center.Add(-1 * xTranslation, 0, -2), center.Add(-1 * xTranslation, 4, 2), block);
			...
			yield return new FillCommand(center.Add(-1 * xTranslation, 1, -1), center.Add(1 * xTranslation, 3 + extraHeight, 1), Block.Air, FillMode.Replace);
		}
		...
		public static IEnumerable<Command> GenerateNetherPortalRoomZ(NetherPortalRoomParameters parameters, Block block)
		{
			...
			yield return new FillCommand(center.Add(-2, 0, -1 * zTranslation), center.Add(2, 4 + extraHeight, -1 * zTranslation), block);
			...
			yield return new FillCommand(center.Add(-1, 1, -1 * zTranslation), center.Add(1, 3 + extraHeight, 1 * zTranslation), Block.Air, FillMode.Replace);
		}
	}
}
